...
export default function Dashboard({ session }) {
  ...
  // imageTexts 已移除（v3 全量读取模式不再需要逐页文字）
  ...
  // 🆕 v3 全量读取模式：仅校验整体故事说明（绘本必填，插画不要求）
  let isPictureBookValid = true;
  let warningMessage = "";

  if (activeTab === 'picturebook' && selectedImages.length > 0) {
    if (imageType === 'picturebook' && (!workText || workText.trim().length === 0)) {
      isPictureBookValid = false;
      warningMessage = `🚨 请在下方填写作品的【整体故事说明】（绘本必填，AI 将与故事还原结果进行比对分析）。`;
    }
  }
              ...
              <div style={{ display: 'flex', gap: '20px', marginBottom: '25px', backgroundColor: '#f8fafc', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                ...
                {activeTab === 'picturebook' && (
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <select value={imageType} onChange={(e) => { setImageType(e.target.value); if (e.target.value === 'illustration') setPageFormat('single'); }} style={{ padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1', backgroundColor: 'white', color: '#1e293b', outline: 'none', cursor: 'pointer' }}>
                ...
                {activeTab === 'picturebook' && imageType === 'picturebook' && (
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <select value={pageFormat} onChange={(e) => setPageFormat(e.target.value)} style={{ padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1', backgroundColor: 'white', color: '#1e293b', outline: 'none', cursor: 'pointer' }}>
                      <option value="single">单页图（每张为单独一页）</option>
                      <option value="spread">跨页合图（每张含左右两页）</option>
                    </select>
                    {pageFormat === 'spread' && (
                      <p style={{ fontSize: '11px', color: '#6366f1', margin: 0, lineHeight: '1.5' }}>
                        ✨ 跨页模式：AI 将重点评审跨页构图张力、装订线两侧视觉平衡与翻页悬念设计。
                      </p>
                    )}
                  </div>
                )}
